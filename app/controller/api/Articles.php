...
class Articles
{
    ...
    public function all(Request $request)
    {
        ...
        $query = Article::whereIn('articles.feed_id', $feedIds)
            ->withJoin(['feed' => ['id', 'title']], 'LEFT');
        
        // 搜索关键词
        if ($keyword) {
            $query->where(function($q) use ($keyword) {
                $q->whereLike('articles.title', "%{$keyword}%")
                  ->whereOr('articles.content', 'like', "%{$keyword}%");
            });
        }
        ...
        foreach ($list as &$item) {
            $item['feed_name'] = $item['feed']['title'] ?? '未知来源';
            unset($item['feed']);
        }
    ...
    public function stars(Request $request)
    {
        ...
        $total = $query->count();
        $list = $query->page($dto->page, $dto->page_size)->select();
    ...
    public function listByTag(Request $request)
    {
        ...
        $articleIds = ArticleTags::where('tag_id', $dto->tag_id)->column('article_id');
    ...
    public function info(Request $request)
    {
        ...
        // 获取文章标签
        $tagIds = ArticleTags::where('article_id', $articleId)->column('tag_id');
        $tags = [];
        if (!empty($tagIds)) {
